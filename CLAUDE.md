...
- **Shortcuts in the left pane are a second, separate binding**: because those `QAction`s live on the tables, a key pressed while the favorites list or the folder tree has focus reaches nothing — and clicking a bookmark or a tree folder navigates the active pane but leaves focus in the left pane. `ui.reload_left_pane_shortcuts` therefore binds the window-level / active-pane-level subset (`LEFT_PANE_ACTIONS` in `ui.py`) onto the left-pane widgets too, tagging each action with the `is_left_pane_shortcut` property so a keymap reload can clear them. Handlers are built by `ui._active_pane_shortcut_handler`, which resolves the action against `self.file_explorer` **when the key is pressed**, not when it is bound — `map_shortcut_name_to_func` returns a method bound to one specific table. Actions needing a file selection (copy, cut, delete, enter, Shift+arrow selection, …) are deliberately left out of that list.
